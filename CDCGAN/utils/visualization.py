...
class Visualization():
    ...
    def plot_generated_images(gen_o: tf.keras.Model, latent_dim:int, n_classes:int,
                               ...
                               training=True, show_title=True, fashion_mnist=False):
        ...
        class_name_dict = {0: 'T-shirt/top', 1:'Trouser', 2 :'Pullover', 
        ...
        for class_ in range(n_classes): 
            ...
            start_ind = class_*n_classes

            for j in range(nrows):
                # Reshape (1, 28,28,1) into (28,28,1)
                generated_digit_image = generated_digit_images[start_ind + j]
                axes[j, class_].set_axis_off()
                axes[j, class_].imshow(generated_digit_image, cmap='gray') 
                # Only plot label on the first row
                if j == 0 and show_title:
                    class_str = class_name_dict[class_] if fashion_mnist else str(class_)
                    axes[j, class_].set_title(class_str)
